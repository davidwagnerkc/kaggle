...
TRAIN_DIR = Path('../input/train_1024') # DATA_DIR / 'train'
TEST_DIR = Path('../input/test_1024') #  DATA_DIR / 'test'
...
CHECKPOINT_PATH = Path('dw_resnet34_all_hpa-best_model-16.pth')
...
VALIDATION_SIZE = .10
VISDOM_ENV_NAME = 'dw_resnet34_all_hpa'
...
print('cuda devices', torch.cuda.current_device())
device
...
val_dl = DataLoader(val_ds, batch_size=BATCH_SIZE, pin_memory=PINNED, num_workers=NUM_WORKERS)
        ...
        for phase in ['train', 'val']:
            ...
            for X, y in tqdm.tqdm(dataloaders[phase]):
                ...
                if phase == 'train':
                    ...
                    f1, best_thresh = 0, 0
                    ycpu = y.cpu()
                    y_cpu = sigmoid(y_.to('cpu'))
                    for thresh in np.linspace(.1, .9, 100):
                        score = f1_score(ycpu, y_cpu > thresh, average='macro')
                        if score > f1:
